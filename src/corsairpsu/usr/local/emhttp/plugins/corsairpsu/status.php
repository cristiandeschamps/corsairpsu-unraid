...
try {
    ...
    if ($settings["TYPE"] === "corsairmi") {
        ...
        // Adds the keys and values to an array, named appropriately
        $json = [
            'temp1' => $data['temp1'],
            'temp2' => $data['temp2'],
    ...
    if ($settings["TYPE"] === "cpsumoncli") {
        $capacity = filter_var($data["PSU type"], FILTER_SANITIZE_NUMBER_INT);
        $input = (float)$data['Input power'];
        $load = round($input / $capacity * 100);

        $json = [
            'voltage' => $data["Voltage"],
            'current' => $data["Current"],
            'temp1' => (float)$data["Temperature"],
            'temp2' => (float)$data["Temperature"],
            'fan_mode' => $data["Fan mode"],
            'fan_rpm' => (float)$data["Fan speed"],
            'capacity' => $capacity,
            'watts' => $input,
            'load' => $load,
            'vendor' => "CORSAIR",
        ...
        // Issue with the rails output
        // @see https://github.com/ka87/cpsumon/pull/2
        $rail_5v = !empty($data['5V Rail']) ? explode(",", $data['5V Rail']) : null;
        $rail_3v = !empty($data['3.3V Rail']) ? explode(",", $data['3.3V Rail']) : null;

        if ($rail_5v && $rail_3v) {
            $rail_5v_watts = round((float)$rail_5v[2], 2);
            $rail_3v_watts = round((float)$rail_3v[2], 2);

            $rail_12v = $input - $rail_5v_watts - $rail_3v_watts;

            $rail_12v_watts = round($rail_12v, 2);

            $rail_5v_load = round((float)$rail_5v[2] / $capacity * 100);
            $rail_3v_load = round((float)$rail_3v[2] / $capacity * 100);
            $rail_12v_load = round($rail_12v / $capacity * 100);
    ...
    // do nothing
    // cpsumoncli is unstable and will sometimes not report.
